...
var currentObj = this
// exports
export class AddPackage extends Form {
  constructor(props) {
    super(props)
    this.state={nameStatus:'', productCodeStatus:'', descriptionStatus:'', itemArray:[], totalPrice:'0.0', name:'', product_code:'', description:'', price:'' , picture: '', product_details:'', checkbox: true}
...
setFieldValue(field, e) {
    ...
    dispatch({
        type: 'SET_ADD_PACKAGE_FIELD_VALUE',
        params: {
        field: field,
        value: value
        }
...
const { form, dispatch, session } = this.props
    ...
    previousItemArray = []
    currentSelectedArray = []
    this.state = {name:'', product_code:'', description:'', price:'' , product_details: '', itemArray:[]}
}
...
return (
	...
	{product.isSubmitting == true ? <loader style={style.loader}><ReloadingGraphic customStyle = {style.loaderImage} /></loader>:''}
	<DashboardHeader/>
	...
	<headerLink className="col-xs-12 col-sm-6 col-md-5 pull-right" style={style.headerLink}>
		<backButton style={style.saveButtonArea}>
			<Link to='/package' style={style.blueButton}>Back</Link>
		</backButton>
	</headerLink>
	...
	<tabArea className="col-sm-8 col-md-9 mobilepadding">
		<Tabs defaultActiveKey={1}>
			<Tab eventKey={1} title="Add Package">
		    	<addPackage className='col-xs-12 col-sm-12 col-md-12' style={[style.packageh1, style.padding0]}> Add Package
		  		</addPackage> 
				...
				<FormInput  customStyle={style.inputBox} type='text' value={form.data.product_code} onChange={(e) => { this.setFieldValue('product_code', e) }} placeholder="e.g. PR1234T"/>
				</FormBlock>
				</FormLine>
		        </inputField>
				...
				<FormTextarea rows='2' value={form.data.description} customStyle={style.inventoryTextarea} onChange={(e) => { this.setFieldValue('description', e) }}/>
				</FormBlock>
				</FormLine>
		        </inputField>
...
<TableHeaderColumn dataSort={true} className='columHeight columnBorder'  width = '100' dataFormat={this.addTextBox} dataField='quantity'>Quantity</TableHeaderColumn>

<TableHeaderColumn hidden={true} dataSort={true} className='columHeight columBorder' width = "100" dataFormat={this.addTextBox} dataField="price"></TableHeaderColumn>

<TableHeaderColumn hidden={true} dataSort={true} className='columHeight columBorder' width = "100" dataFormat={this.addTextBox} dataField="id"></TableHeaderColumn>
...
<headingButton style={style.saveProducts}> 
<button style={[style.greenBtn, style.btnSave1]} value="" type="button" onClick={(e) => { this.submit(e) }} >Save Package</button> 
</headingButton> 
...
export default connect(mapStateToProps)(Radium(AddPackage))
